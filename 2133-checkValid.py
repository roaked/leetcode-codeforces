...
class Solution:
    def checkValid(self, matrix: list[list[int]]) -> bool:
        return all(len(set(row)) == len(matrix) for row in (*matrix, *zip(*matrix)))
        # return True if all conditions are met -> length of unique elements = elements of matrix and using * unpacking operator and
        # columns to rows
